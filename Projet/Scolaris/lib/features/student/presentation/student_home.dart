...
class _StudentDashboardState extends ConsumerState<_StudentDashboard> {
  ...
  void _navTo(String labelKey) =>
      ref.read(shellNavProvider.notifier).state = labelKey;
      ...
      child: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          _PremiumShortcutsGrid(onTap: {
            'notes':         () => _navTo('nav.grades'),
            'edt':           () => _navTo('nav.schedule'),
            'devoirs':       () => _navTo('nav.homework'),
            'presences':     () => _navTo('nav.attendance'),
            'cours':         () => _navTo('nav.courses'),
            'messages':      () => _navTo('nav.messages'),
            'bulletin':      () => _navTo('nav.bulletin'),
            'bibliotheque':  () => _navTo('nav.library'),
            'features':      () => _navTo('nav.features'),
          }),
          ...
          _SectionHeader(
            ...
            action: 'Voir tout',
            onAction: () => _navTo('nav.attendance'),
          ),
          const SizedBox(height: 10),
          _AttendanceSummaryCard(onTap: () => _navTo('nav.attendance')),
          const SizedBox(height: 22),
          ...
          // ── 12. Bulletin résumé ───────────────────────────────────
          _BulletinSummaryCard(onTap: () => _navTo('nav.bulletin')),
          const SizedBox(height: 22),
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        ...List.generate(values.length, (i) {
          final v = values[i];
          final barColor = v >= 16 ? _green : v >= 12 ? _gold : _terra;
          return Padding(
