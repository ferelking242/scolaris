...
**Site vitrine réel** = `site_saas/index.html` (HTML/CSS/JS vanilla, config dans un `<script>` en bas de page) — c'est celui-là qu'il faut modifier, PAS le dossier `site/` (mockup Next.js séparé, prix périmés Simple/Pro/Max jamais mis à jour vers le modèle 4 offres, non branché à l'inscription réelle). Le formulaire d'inscription réel redirige vers l'app (`#/register-school`), pas de choix mensuel/annuel à l'inscription — le choix d'offre/période se fait après coup dans `admin_subscription_page.dart`.
